...
> 本集校稿已於本檔建立前完成（2026-08，commit 7ccea6b），本檔為事後補建，供後續集數與回溯參考。

### 補上原文括註：已完成（2026-08-09 人工登記，2026-08-10 執行完畢）
- **背景**：2026-08-09 人工裁決**譯文應保留「中譯（English）」全形括註**，`configs/proofread_guidelines_template.md` 已明訂此規則；格式參考 UFO-05 定稿
- **git 查證結果**：經比對校稿前版本（`git show 560ed72:data/UFO-01/drafts/`），本集草稿於翻譯階段本無括註（全 6 檔僅 1 處），故本任務為**新增**而非還原
- **執行結果（2026-08-10）**：全 6 檔共補 56 處全形括註（39 個 `→` 行），僅補重要人名／地名／機構／計畫／作品名的**全集首次出現**；既有 12 處半形括註（Sagan、藍皮書計畫等）依準則保留原樣；保留原文的知名縮寫（CIA、HBO、MUFON 等）未加註。全部 746 行 `→` 行 JSON 驗證通過
- **新發現並已處理**：seg 361、363 原文 Peter Gerstin 為 **Peter Gersten**（1970 年代末首位提出 UFO 資訊自由法申請的紐約律師）之 Whisper 誤聽，括註採正確拼寫，topics.json 已補錄 2 筆 potential_errors
- **收尾狀態**：`fix_transcription_errors.py` 已執行（13 筆修正入 main.yaml 原文，含 Gersten 2 筆；另 15 筆為前次校稿已修之重複紀錄，屬預期）；backfill／export SRT **刻意暫緩**，待全 20 集校稿完成後由人工統一執行

...
- **git 查證結果**：經比對校稿前版本（`git show 560ed72:data/UFO-01/drafts/`），本集草稿於翻譯階段本無括註（全 6 檔僅 1 處），故本任務為**新增**而非還原；若對個別段落有疑慮，可用 `git log -p -- data/UFO-01/drafts/<file>` 追查該行歷史
- **任務**：於 `data/UFO-01/drafts/` 各檔的定譯中，為重要人名／地名／專有名詞補上原文括註（例：艾德加·米切爾博士（Dr. Edgar Mitchell）、羅斯威爾（Roswell））。僅補重要專有名詞的**首次出現**，無需重複
